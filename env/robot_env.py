...
class PtdrlRobEnv():
    ...
    def __init__(self, model_name, amcl, min_dist_to_obstacle, min_dist_to_goal, num_tracks, timeout):
        ...
        # Global path
        print("Suscribing to global path")
        rospy.Subscriber("/move_base/NavfnROS/plan", Path, self._global_path)
        print("Suscribed!")

        # Global path
        print("Suscribing to cmd_vel_dwa")
        rospy.Subscriber("/pedbot/control/cmd_vel_dwa", Twist, self._cmd_vel_dwa) 
        print("Suscribed!")
    ...
    def tune_parameters(self, params):
        ...
        # cf = self._tuning_client_local.update_configuration({})
        # print(cf)
        if inflation:
            self._tuning_client_inflation.update_configuration(inflation_param)
            # ci = self._tuning_client_inflation.update_configuration({})
            # print(ci)
    
    def get_parameters(self):
        cg = copy.deepcopy(self._tuning_client_local.get_configuration())
        cg_inflation = copy.deepcopy(self._tuning_client_inflation.get_configuration())
        return cg, cg_inflation
    ...
    def get_costmap(self):
        ...
        #print(f"costmap.data : {len(costmap.data)}")
    ...
    def get_cmd_vel_dwa(self):
        cmd_vel_dwa = copy.deepcopy(self.cmd_vel_dwaa)
        return cmd_vel_dwa
