...
# Initialize WandB
if USE_WANDB:
	wandb.init(project="AAAI TD3+OE", name="APPLR discount 0.95, long abort, penalize stopping near walls, more people")
...
curriculum[0,0] = 6e3
curriculum[0,1] = 800 #400

curriculum[1,0] = 200e3
curriculum[1,1] = 800 #400


curriculum_counter = 0
curriculum_goals = 2
max_time_steps = 800 #400
...
def eval_policy(policy, env, eval_episodes=6):
	...
	for _ in range(eval_episodes):
		...
		while not done:
			...
			vel, goal, state, reward, done = env.step_cdrl_short_lidar(action)
			avg_reward += reward
...
def main():
	...
	parser.add_argument("--start_timesteps", default=0*5e3, type=int)# Time steps initial random policy is used 25e3 ### 50e3, 2e3, 8e3
	...
	state_dim = 720 * 4 # 120 Latent space 10. 10x4=40
	action_dim = 7 # 2
	max_action = 2.0 # Maybe separate linear and angular vel 2.0
	...
	if args.policy == "TD3":
		...
		policy = TD3_cdrl_try_rl.TD3(**kwargs)
	...
	replay_buffer = utils_TD3_cdrl_try.ReplayBufferLidar(state_dim, action_dim)
